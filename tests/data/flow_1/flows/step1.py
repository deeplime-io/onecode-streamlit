...
import onecode_st as oc_st


def run():
    df = oc_st.csv_reader("csv", "input_test.csv")

    if df is not None:
        col_X = oc_st.dropdown('Column X', 'x', options='$csv$.columns', multiple=False)

        # line commented to test and ensure it is not interpreted by OneCode
        # multi = oc_st.dropdown('XYZ', 'ni', options='$csv$.columns', multiple=True)
    ...
    oc.Logger.info(oc_st.csv_output('My CSV', 'test.csv'))
